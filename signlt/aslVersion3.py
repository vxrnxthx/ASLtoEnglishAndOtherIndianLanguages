...
def add_space(word_box, sentence_box):
...
def pipe_cam(gui, vid_label, left_frame):
    ...
    labels = ['threshold', 'current char', 'original word', 'sentence']
    ...
    add_space_btn = Button(control_frame, text="Add Space", width=15,
                           command=lambda: add_space(entryboxes['original word_entrybox'],
                                                     entryboxes['sentence_entrybox']))  # Adjusted call
    ...
    # Run video stream
    names = ['th_box', 'vid_label', 'hands', 'cc_box', 'cc_pred_box']
    kwargs = {
        'th_box': entryboxes['threshold_entrybox'],
        'vid_label': vid_label,
        'hands': mp_hands.Hands(min_detection_confidence=0.8, min_tracking_confidence=0.8),
        'cc_box': entryboxes['current char_entrybox'],
        'cc_pred_box': entryboxes['current char_entrybox']
    }

    frame_video_stream(names, curr_char, prev_char, *list(kwargs.values()))
    gui.root.mainloop()
